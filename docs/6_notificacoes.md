---
sidebar_position: 11
title: Notificações Push & Regras
---
...
> [!NOTE]
> Salvo indicação em contrário, todas as notificações são destinadas diretamente ao motorista responsável (`user_tax_id`).
...
| `ANNOUNCEMENT` | *Título do Anúncio* | Painel Web (`POST /announcements`) | Comunicar avisos gerais e alertas do terminal. |
...
sequenceDiagram
    ...
    participant L as Appointments Logs
    participant FCM as Firebase FCM / Mobile

    S->>DB: Query agendamentos ativos na janela
    DB-->>S: Retorna lista de agendamentos
    S->>L: Consulta logs de notificações já enviadas para os agendamentos (event='notification_sent')
    L-->>S: Retorna IDs com push_type='WINDOW_OPEN'
    loop Para cada agendamento
        alt ID já presente nos logs enviadas
            S->>S: Ignora envio (já notificado)
        else ID novo
            S->>FCM: Dispara push notification
            S->>L: Grava log com event='notification_sent' e json.push_type='WINDOW_OPEN'
        end
    end
```
